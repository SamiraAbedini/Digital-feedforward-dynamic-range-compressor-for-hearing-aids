function y = DecoupledSmoothPeakDetector(x, fs, AttackTime, ReleaseTime)
% =========================================================================
%              Decoupled Smooth Peak Detector  
% =========================================================================
%
% This program contains one main stage of Decoupled Smooth Peak detection
% Decoupled Smooth Peak detection Procedure:
% -------------------------------------------------------------------------
%   Usage:  
%      y = DecoupledSmoothPeakDetector(x, fs, AttackTime, ReleaseTime)
% -------------------------------------------------------------------------
%   Inputs:
...
%      y = DecoupledSmoothPeakDetector(x, fs, 0.02, 0.02)
% -------------------------------------------------------------------------
%   References:
...
for i=1:length(x)
    if y1(i) > state
        ...
        state =alphaRel * state + (1-alphaRel) *y1(i);
    end
    y1(i) = state;
end
...
subplot(2,1,2)   %plotting the decoupled smooth peak detector output
y2 = y;
plot(t,y2,'R')
title('Subplot 2: Decoupled Smooth Peak Detector')