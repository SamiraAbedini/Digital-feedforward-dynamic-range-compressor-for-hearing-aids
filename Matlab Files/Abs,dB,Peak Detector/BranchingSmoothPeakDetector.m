function y = BranchingSmoothPeakDetector(x, fs, AttackTime, ReleaseTime)
% =========================================================================
%              Branching Smooth Peak Detector  
% =========================================================================
%
% This program contains one main stage of Branching Smooth Peak detection
% Branching Smooth Peak detection Procedure:
% -------------------------------------------------------------------------
%   Usage:  
%      y = BranchingSmoothPeakDetector(x, fs, AttackTime, ReleaseTime)
% -------------------------------------------------------------------------
%   Inputs:
...
%      y = BranchingSmoothPeakDetector(x, fs, 0.02, 0.02)
% -------------------------------------------------------------------------
%   References:
...
for i=1:length(x)
    if y(i) > state
        ...
        state =alphaRel* state + (1-alphaRel) * y(i) ;
    end
    y(i) = state;
end
% -------------------------------------------------------------------------
...
subplot(2,1,2)   %plotting the branching smooth peak detector output
y2 = y;
plot(t,y2,'R')
title('Subplot 2: Branching Smooth Peak Detector')